...
def reverse_KLLoss(input_, target_, coeff, args):
    softmax = nn.Softmax(dim=1)(input_)

    kl_loss = (- softmax * torch.log(target_ + args.epsilon2)).sum(dim=1)
    # kl_loss *= coeff
...
def mixup(x, c_batch, t_batch, netF, netB, netC, args):
    ...
    if args.alpha == 0:
        ...
        loss1 = KLLoss(outputs, t_batch, c_batch, args)
        loss2 = reverse_KLLoss(outputs, t_batch, c_batch, args)
        return 1.0 * loss1 + 0.1 * loss2

    lam = (torch.from_numpy(np.random.beta(args.alpha, args.alpha, [len(x)]))).float().cuda()
    t_batch = torch.eye(args.class_num)[t_batch.argmax(dim=1).cpu()].cuda()
    ...
    loss1 = KLLoss(mixed_outputs, mixed_t, mixed_c, args)
    loss2 = reverse_KLLoss(mixed_outputs, mixed_t, mixed_c, args)
    return 1.0 * loss1 + 0.1 * loss2
...
if __name__ == "__main__":
    ...
    parser.add_argument('--output', type=str, default='ckps/time3/')
    parser.add_argument('--output_src', type=str, default='ckps/src3/')
    ...
    if args.dset == 'office-home':
        ...
        # args.names = ['Clipart', 'Art', 'Product', 'RealWorld']
        args.class_num = 65
    if args.dset == 'office':
