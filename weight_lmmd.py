...
class weight_lmmd(MMDLoss, LambdaSheduler):
    def __init__(self, num_class, kernel_type='rbf', kernel_mul=2.0, kernel_num=5, fix_sigma=None,
                 gamma=1.0, max_iter=1000, **kwargs):
        '''
        Local MMD
        '''
        super(weight_lmmd, self).__init__(kernel_type, kernel_mul, kernel_num, fix_sigma, **kwargs)
        super(MMDLoss, self).__init__(gamma, max_iter, **kwargs)
        self.num_class = num_class

    def forward(self, source, target, source_predict, tgt_predict, src_coeff, tgt_coeff):
        if self.kernel_type == 'linear':
            raise NotImplementedError("Linear kernel is not supported yet.")

        elif self.kernel_type == 'rbf':
            batch_size = source.size()[0]
            weight_ss, weight_tt, weight_st = self.cal_weight(source_predict, tgt_predict, src_coeff, tgt_coeff)
    ...
    def cal_weight(self, source_label, target_logits, src_coeff, tgt_coeff):

        batch_size = source_label.size()[0]

        # 权重
        src_coeff = src_coeff.reshape(batch_size, 1)
        src_coeff = src_coeff.cpu().data.numpy()

        tgt_coeff = tgt_coeff.reshape(batch_size, 1)
        tgt_coeff = tgt_coeff.cpu().data.numpy()

        source_label = source_label.cpu().data.numpy()
        source_label_onehot = np.eye(self.num_class)[source_label]  # one hot

        source_label_onehot *= src_coeff
        ...
        target_logits = target_logits * tgt_coeff
